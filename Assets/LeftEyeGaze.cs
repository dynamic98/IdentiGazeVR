...
public class LeftEyeGaze : MonoBehaviour
{
    ...
    // 모든 로그 데이터를 저장할 리스트
    public List<string> LeftlogData = new List<string>();
    ...
    void Update()
    {
        ...
        if (isHMDWorn != wasHMDWorn)
        {
            if (!isHMDWorn)
            {
                LogHMDRemoved(); // 헤드셋이 벗겨진 상태를 리스트에 저장
            }
            else
            {
                LogHMDWorn(); // 헤드셋이 다시 착용된 상태를 리스트에 저장
            }
            wasHMDWorn = isHMDWorn;
        }

        // HMD를 착용하고 있지 않다면 시선 데이터를 처리하지 않음
        if (!isHMDWorn || !isRecording || (leftEyeGaze == null)) return;
        ...
        // 시선 데이터 수집
        Vector3 gazeOrigin = leftEyeGaze.transform.position;
        ...
        else
        {
            ...
            Debug.Log("Left Eye Gaze Position: " + gazePoint);
        }
    ...
    public void StopRecording()
    {
        ...
        // 모든 로그 데이터를 파일에 저장
        File.WriteAllLines(filePath, LeftlogData);
