...
public class LeftEyeGaze : MonoBehaviour
{
    public OVREyeGaze leftEyeGaze;
    public OVRFaceExpressions faceComponent;

    // LineRenderer를 각각의 눈에 대해 추가
    private LineRenderer leftEyeLineRenderer;
    ...
    private List<Vector3> leftEyeGazeDataList = new List<Vector3>();

    void Start()
    {
        // 파일 경로 설정
        filePath = Path.Combine(Application.persistentDataPath, "LeftEyeTrackingLog.txt");
        Debug.Log(filePath);
        File.WriteAllText(filePath, "Left Gaze Data Log\n");

        // LineRenderer 설정
        leftEyeLineRenderer = CreateLineRenderer(Color.blue);
    }

    void Update()
    {
        if (!isRecording || (leftEyeGaze == null)) return;

        // 눈 깜박임 상태 확인
        float leftBlinkWeight;
        faceComponent.TryGetFaceExpressionWeight(OVRFaceExpressions.FaceExpression.EyesClosedL, out leftBlinkWeight);

        // 눈이 감겨있는지 확인 (예: 0.5 이상이면 반 이상 감겨있는 상태로 간주)
        if (leftBlinkWeight >= 0.5f)
        {
            Debug.Log("Left eye is closed. Logging closed state.");
            LogEyeClosed();  // 눈이 감긴 상태를 파일에 기록
            return;
        }
        // 오른쪽 눈 시선 데이터 처리
        if (leftEyeGaze != null && leftEyeGaze.EyeTrackingEnabled)
        {
            ProcessGazeData(leftEyeGaze, leftEyeGazeDataList, "Left Eye", leftEyeLineRenderer);
        }
    ...
    void LogEyeClosed()
    {
        ...
        string log = currentTime + " - Left eye is closed.\n";
    ...
    void SaveGazeData()
    {
        ...
        string log = currentTime + " - Left Eye Gaze Data:\n";
        foreach (Vector3 gazePoint in leftEyeGazeDataList)
        {
            log += "Left Eye Gaze Position: " + gazePoint.ToString() + "\n";
        }
